...
inline int TemperatureTableModel::rowCount(const QModelIndex &parent /* = QModelIndex() */) const
{
...
inline int TemperatureTableModel::columnCount(const QModelIndex &parent /* = QModelIndex() */) const
{
    // check if the parent index is valid (belongs to a model and doesn't have negative row & column number)
    // if it is valid, the current modelindex shouldn't exist since it's not a tree (the index don't have children)
    if(parent.isValid()) return 0;
...
inline Qt::ItemFlags TemperatureTableModel::flags(const QModelIndex & index) const
{
